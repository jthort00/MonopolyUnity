...
char* CreateGame (char username[256], int socket, ListGames *listgames)
{
	...
	sprintf(consulta4, "SELECT propertyID FROM Properties");
	printf("consulta = %s\n", consulta4);
	...
	row = mysql_fetch_row (resultado);
	while(row!=NULL) {
		sprintf(consulta1, "SET FOREIGN_KEY_CHECKS=0");
		err = mysql_query(conn, consulta1);
		sprintf(consulta1, "INSERT INTO PropertyAssign VALUES (%d, '-1', %s, 0, 0)", newgameid, row[0]);
		printf("consulta = %s\n", consulta1);
		err = mysql_query(conn, consulta1);
		if (err!=0) {
			printf ("Error al introducir datos la base %u %s\n", 
...
int AddUsertoGame (int gameid, char nombre[512], int socket, ListGames *listgames)
{
	...
	if (err!=0) {
		...
		printf("DID WORK\n");
		sprintf(consulta2, "INSERT INTO PlayerStatus VALUES (%d, '%s', 2000, 0, 0, 0, 0)", gameid, nombre);
		printf("consulta = %s\n", consulta2);
...
int FindGameInList (int gameid, ListGames *listgames)
{
	int i =0;
	int found =0;
	while (i<listgames->num && found==0)
	{
		if (listgames->game[i].gameid == gameid)
		{
			found = 1;
		}
		else
...
int MoneyUpdate (char username[256], int gameid, int money)
{
	
}

char* PropertySituation (char username[256], int gameid, int position)
{
	int err;
	char consulta[512];
	char consulta1[512];
	...
	sprintf (consulta, "SELECT PropertyAssign.playerID, Properties.price, Properties.basic_fee, Properties.building_fee, Properties.name FROM PropertyAssign, Properties WHERE PropertyAssign.gameID = %d AND Properties.propertyID = PropertyAssign.propertyID AND Properties.location=%d;" , gameid, position);
	printf("consulta = %s\n", consulta);
	err = mysql_query(conn, consulta);
	resultado = mysql_store_result (conn);
	row = mysql_fetch_row (resultado);
	...
	char result[512];
	sprintf(result, "%d/%d/%d/%d/%d", row[0], row[1], row[2], row[3], row[4]);
	return result;
	
	
}

int PositionUpdate (char username[256], int gameid, int dices)
{
	int err;
	char consulta[512];
	char consulta1[512];
	...
	sprintf (consulta, "SELECT position FROM PlayerStatus WHERE gameID = %d AND playerID = '%s';", gameid, username);
	printf("consulta = %s\n", consulta);
	err = mysql_query(conn, consulta);
	resultado = mysql_store_result (conn);
	row = mysql_fetch_row (resultado);
	...
	int current_pos = atoi(row[0]);
	int new_pos = current_pos+dices;
	if (new_pos>=40)
		new_pos=new_pos-40;
	sprintf(consulta1, "UPDATE PlayerStatus SET position = %d WHERE gameID = %d AND playerID = '%s';",new_pos, gameid, username);
	printf("consulta1 = %s\n", consulta1);
...
void *AtenderCliente (TParam *par)
{
	...
	while (end==0){
		...
		if (codigo ==8) { //Respuesta invitación
			...
			int ad = atoi(p);
			int j=0;
			int i = FindGameInList(gid, par->listgames);
			if (ad==0)
			{
...
/*		}*/
		if (codigo==10) { //Siguiente turno mensaje recibido con la estructura 10/username/gameid
			p = strtok(NULL, "/");
			int gid = atoi(p);
			int index = FindGameInList(gid, par->listgames);
			int found = 0; 
			int j = 0;
			int k;
			while (j<par->listgames->game[index].num && found == 0)
			{
				k = strcmp(par->listgames->game[index].ingame_users[j].name, nombre);
				if (k==0)
				{
					found = 1;
					j=j+1;
					if (j==par->listgames->game[index].num)
						j=0;
				}
				else
					j=j+1;
			}
			sprintf(respuesta, "10?%d/%s", gid, par->listgames->game[index].ingame_users[j].name);
			j=0;
			while (j<par->listgames->game[index].num)
			{
				write(par->listgames->game[index].ingame_users[j].socket, respuesta, strlen(respuesta));
				j=j+1;
			}
			printf("%s\n",respuesta);
		}
		...
		if (codigo==12) { //Añadir/Quitar dinero 12/username/gameid/money 
			p = strtok(NULL, "/");
			int gid = atoi(p);
			p = strtok(NULL, "/");
		...
		if (codigo==13) { //Actualizar posicion 12/username/gameid/dices 
			p = strtok(NULL, "/");
			int gid = atoi(p);
			p = strtok(NULL, "/");
			int dices = atoi(p);
			int index = FindGameInList(gid, par->listgames);
			int newpos = PositionUpdate(nombre, gid, dices);
			sprintf(respuesta, "13?%s/%d/%d", nombre, gid, newpos);
			int j=0;
			while (j<par->listgames->game[index].num)
			{
				write(par->listgames->game[index].ingame_users[j].socket, respuesta, strlen(respuesta));
		...
		if  (codigo==14) { //Devolver situación de la casilla en la que se encuentra el jugador
			p = strtok(NULL, "/");
			int gid = atoi(p);
			p = strtok(NULL, "/");
			int position = atoi(p);
			char* situation = PropertySituation(nombre, gid, position);
			sprintf(respuesta, "14?%s/%d/%s", nombre, gid, situation);
			write (sock_conn,respuesta, strlen(respuesta));
		}
